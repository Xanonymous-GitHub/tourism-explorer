        ...
        <div className="px-10">
            <div className="dropdown inline-block relative">
                <button className="bg-gray-300 text-gray-700 py-2 px-4 rounded inline-flex items-center">
                    <span className="mr-1">{currentCity}</span>
                        ...
                        cities.map(city =>
                            city.name !== currentCity ?
                                <Link to={'/ScenicSpot/' + city.id} key={city.id}>
                                    <li className="bg-gray-200 hover:bg-gray-400 active:bg-gray-500 py-2 px-4 block whitespace-no-wrap select-none cursor-pointer">
                                        {city.name}
                                    </li>
                                </Link> :
                                <li key={city.id}
